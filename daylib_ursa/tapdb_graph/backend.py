...
URSA_TEMPLATE_DEFINITIONS: list[TemplateSpec] = [
    ...
    TemplateSpec("RGX/run-directory/mount/1.0/"),
    TemplateSpec("RGX/run-directory/mount-event/1.0/"),
    TemplateSpec("RGX/analysis/run-job/1.0/"),
    TemplateSpec("RGX/analysis/run-job-event/1.0/"),
...
class TapDBBackend:
    ...
    def __init__(
        ...
        *,
        ...
        resolved_client_id = (
            client_id or str(getattr(settings, "tapdb_client_id", "") or "").strip()
        )
        if not resolved_client_id:
            raise RuntimeError("Ursa TapDB client_id is required")
        resolved_namespace = (
            namespace or str(getattr(settings, "tapdb_database_name", "") or "").strip()
        )
        if not resolved_namespace:
            raise RuntimeError("Ursa TapDB database_name is required")
        resolved_app_username = (
            app_username or str(getattr(settings, "tapdb_client_id", "") or "").strip()
        )
