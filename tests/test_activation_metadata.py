...
def test_pyproject_contains_the_single_python_install_set() -> None:
    ...
    for expected in (
        ...
        "daylily-ephemeral-cluster==5.0.31",
        "daylily-tapdb==7.0.9",
        "fastapi>=0.104.0",
        "httpx>=0.25.0",
        "itsdangerous>=2.2.0",
...
def test_ecosystem_versions_track_ephemeral_cluster_baseline() -> None:
    ...
    assert payload["components"]["daylily-ephemeral-cluster"]["current"] == "5.0.31"
    assert (
    ...
    assert payload["tested_combinations"][0]["ephemeral_cluster"] == "5.0.31"
