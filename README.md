...
- `daylily-ephemeral-cluster==3.0.0`
...
Manifest creation rejects caller-supplied generated manifest metadata. Ursa derives `analysis_samples_manifest` from `editor_analysis_inputs` or S3 input references, using the installed lsmc-bio `daylily-ephemeral-cluster` `3.0.0` template.
